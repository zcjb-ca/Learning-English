...
1. **上传 PDF**：在首页选一个 PDF 上传，几十秒后会自动生成一节课（抽出可理解段落 + 高频可迁移句式）。**以后想加新材料，随时在应用里上传即可，不用改代码、不用重新部署。**
2. **进入课程，按五步练**：
   1. **读**：听示范朗读（可变速），把材料读懂、跟读。
   2. **句式**：看本课抽出的高频句式和例句。
...
- **上传 PDF 报"提取不到文字"？** 这个 PDF 可能是扫描的图片版（没有文字层），换一个带文字的 PDF。
